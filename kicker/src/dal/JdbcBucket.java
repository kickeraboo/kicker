...
public class JdbcBucket implements BucketDao {
	 ...
	 * 
	 * @param newBucket
	 *            The bucket instance to be inserted.
	...
	public Bucket createBucket(Bucket newBucket) {
		...
		query = String.format(query, newBucket.getUserID(),
				newBucket.getBucketName(), newBucket.getDescription(),
				newBucket.getCityID());
	 ...
	 * 
	 * @param newBucket
	 *            The bucket instance to be updated.
	...
	public boolean updateBucket(Bucket newBucket) {
		...
		query = String.format(query, newBucket.getUserID(),
				newBucket.getBucketName(), newBucket.getCreationDate(),
				newBucket.getDescription(), newBucket.getCityID(),
	 ...
	 * 
	 * @param oldBucket
	 *            The bucket instance to be deleted.
	 * @return Returns true if the bucket was deleted and false if the bucket
	 *         was not deleted.
	 */
	@Override
	 ...
	 * 
	 * @param bucketId
	 *            The id of the bucket to be deleted in the database.
	 * @return Returns true if the bucket was deleted and false if the bucket
	 *         was not deleted.
	 */
	@Override
	...
	/**
	 * convertResultSetToBucket for lack of implicit/explicit operators, we
	 * manually convert a row in the result set, into a bucket.
	 * 
	 * @param rs
	 *            The result set to convert into a Bucket object.
	...
	public Bucket convertResultSetToBucket(ResultSet rs) {
		if (rs != null) {
			try {
				...
				return new Bucket(pBucketID, pUserID, pBucketName,
						pCreationDate, pDescription, pCityID);
			} catch (Exception e) {
				// TODO: Handle
			}
		}
	...
	public List<Bucket> getBucketsByUserId(int userId) {
		ResultSet rs = DataService
				.getData("SELECT * FROM Buckets WHERE UserID = " + userId);
