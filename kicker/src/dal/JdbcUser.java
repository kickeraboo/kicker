...
public class JdbcUser implements UserDao
{
    ...
    * 
    * @param newUser
    *           The user instance to be inserted.
   ...
   public User createUser(User newUser)
   {
      ...
      query = String.format(query, newUser.getEmail(), newUser.getFacebookID(), newUser.getUsername(), newUser.getFullname(), newUser.getRoleID(),
   ...
   {
      // format the string
      String query = "UPDATE Users SET Email = '%1$s', FacebookID = '%2$s', Username = '%3$s', RoleID = %4$d, Status = %5$d, Fullname = '%7$s' WHERE UserID = %6$d";

      query = String.format(query, newUser.getEmail(), newUser.getFacebookID(), newUser.getUsername(), newUser.getRoleID(), (newUser.getStatus() ? 0
            : 1), newUser.getUserID(), newUser.getFullname());
    ...
    * @return Returns true if the user was deleted and false if the user was not
    *         deleted.
    */
   @Override
    ...
    * 
    * @param userId
    *           The id of the user to be deleted in the database.
    * @return Returns true if the user was deleted and false if the user was not
    *         deleted.
    */
   @Override
   ...
   /**
    * convertResultSetToUser for lack of implicit/explicit operators, we
    * manually convert a row in the result set, into an user.
    * 
    * @param rs
    *           The result set to convert into a User object.
   ...
   public User convertResultSetToUser(ResultSet rs)
   {
      if (rs != null)
      {
         try
         {
            ...
            String pFullname = rs.getString("Fullname");
            int pRoleID = rs.getInt("RoleID");
            boolean pStatus = rs.getBoolean("Status");

            return new User(pUserID, pEmail, pFacebookID, pUsername, pFullname, pRoleID, pStatus);
         }
         catch (Exception e)
         {
            // TODO: Handle
